...
public class AggregationClient{
  ...
  private final HBaseClientImpl client;

  public AggregationClient(HBaseClientImpl client) {
    this.client = client;
  }
  ...
  public <R, S, P extends Message, Q extends Message, T extends Message> R max(
      final String tableName, final ColumnInterpreter<R, S, P, Q, T> ci, final Scan scan)
  throws Throwable {
    R res = null;
    Table table = client.getTable(tableName);
    if(table != null){
      res = max(table, ci, scan);
      table.close();
    }
    return res;
  }
  ...
  private <R, S, P extends Message, Q extends Message, T extends Message> Pair<S, Long> getAvgArgs(
      final String tableName, final ColumnInterpreter<R, S, P, Q, T> ci, final Scan scan)
      throws Throwable {
    Pair<S, Long> res = null;
    Table table = client.getTable(tableName);
    if (table != null) {
      res = getAvgArgs(table, ci, scan);
      table.close();
    }
    return res;
  }
   ...
   */
  public <R, S, P extends Message, Q extends Message, T extends Message>
  double avg(final String tableName,
   ...
   */
  public <R, S, P extends Message, Q extends Message, T extends Message>
  double std(final String tableName, ColumnInterpreter<R, S, P, Q, T> ci,
      Scan scan) throws Throwable {
    double res = 0;
    Table table = client.getTable(tableName);
    if (table != null) {
      res = std(table, ci, scan);
      table.close();
    }
    return res;
  }
   ...
   */
  public <R, S, P extends Message, Q extends Message, T extends Message>
  R median(final String tableName, ColumnInterpreter<R, S, P, Q, T> ci,
      Scan scan) throws Throwable {
    R res = null;
    Table table = client.getTable(tableName);
    if (table != null) {
      res = median(table, ci, scan);
      table.close();
    }
    return res;
  }
  ...
  R median(final Table table, ColumnInterpreter<R, S, P, Q, T> ci,
    ...
    if (startRow != null) scan2.withStartRow(startRow);
